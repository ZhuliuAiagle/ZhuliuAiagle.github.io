...
* `#w`会将光标向前移动#个单词的距离，停在第#个单词的开头；
* `#e`会将光标移动到包含当前所在单词的第#个单词的末尾（如果光标在单词最末尾，则移动到下一个单词）；
* 输入`0`会将光标移动到一行的开头。
...
* `o`：可在当前光标悬停行的后面插入一个空行并进入INSERT模式；
* `O`：可在当前光标悬停行的前面插入一个空行并进入INSERT模式。

### 大写R替换文本

按动大写`R`可以使得编辑器进入REPLACE模式，REPLACE模式下在光标闪动处输入文本可以替换当前位置文本。REPLACE模式同样需要按动`ESC`键退出。

### 复制粘贴

* `y`可以复制在VISUAL模式下被选中的文本；
* `p`可以粘贴剪贴板中的文本至光标当前位置之后；
* `y`可以搭配`<MOTION>`来使用，如`yw`不需要在VISUAL模式下就可以复制整个单词。

### 个性化搜索

* `set ic`：Ignore Case，表示搜索中大小写不敏感；
* `set noic`：表示设置搜索中大小写敏感；
* `set hls`：所有搜索结果高亮，`nohls`取消；
* `set is`：开启递进搜索，即随着你字串的变化，动态显示搜索结果，如同搜索引擎一样。`nois`取消；
* `/ignore\c`：如果你只想在一次查询中无视大小写，请使用`\c`标注（这里没太理解）。
